...
app = Flask('')
@app.route('/')
def home(): return "Fasil Bingo is Active & Stable! 🔥"
...
def load_data():
    ...
    try:
        ...
        if record: data.update(record["content"]); return True
    except Exception as e: print(f"❌ Load Error: {e}")
    return False

# --- 4. Board System (Fixed Error 400) ---
def update_group_board(b_id):
    ...
    # CRITICAL: Always include the keyboard to avoid Error 400
...
def callback_listener(call):
    ...
    elif call.data.startswith('approve_') and call.from_user.id in ADMIN_IDS:
...
def finalize_app(message, target):
    try:
        ...
        bot.send_message(message.chat.id, "✅ ተፈጽሟል።")
...
def handle_receipts(message):
    ...
    bot.send_message(uid, "⏳ ደረሰኝዎ ደርሶኛል፣ እየታየ ነው...")

# --- 6. Polling & Keep Alive ---
def ping_self():
    while True:
        try: requests.get("https://fasil-bingo.onrender.com")
        except: pass
        time.sleep(300)
